...
write.csv(DM_tidy, file="DM_tidy_table.csv")
...
write.csv(DM_tidy_PPH, file="DM_tidy_Pero_Plasmo_Hyalo.csv")

DM_tidy_others <-subset(DM_tidy, !genus=="Hyaloperonospora" & !genus=="Peronospora" & !genus=="Plasmopara")

write.csv(DM_tidy_others, file="DM_tidy_others.csv")
...
ppi <- 300
tiff("mycorrhizae_plots_stacked.tiff", width= 7*ppi, height=9.7*ppi, res = ppi, compression= "lzw")
mycorr_PPH_plot /mycorr_others_plot
dev.off()
...
tiff("photosynthesis_plots_stacked_bold.tiff",  width= 7*ppi, height=9.7*ppi, res = ppi, compression= "lzw")
photo_PPH_plot / photo_others_plot
dev.off()
